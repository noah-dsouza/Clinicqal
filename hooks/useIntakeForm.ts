...
import { IntakeFormData, defaultIntakeFormData } from "@/lib/digital-twin/schema";
...
export function useIntakeForm(onSuccess?: () => void): UseIntakeFormReturn {
  ...
  const updateDemographics = useCallback(
    (updates: Partial<IntakeFormData["demographics"]>) => {
      setFormData((prev) => ({
        ...prev,
  ...
  const updateLifestyle = useCallback(
    (updates: Partial<IntakeFormData["lifestyle"]>) => {
      setFormData((prev) => ({
        ...prev,
  ...
  const submitForm = useCallback(async () => {
    ...
    try {
      ...
      const data: { twin: import("@/lib/digital-twin/schema").DigitalTwin } = await response.json();
      setTwin(data.twin);
      onSuccess?.();
    } catch (err) {

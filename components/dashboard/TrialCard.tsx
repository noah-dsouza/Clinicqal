...
export function TrialCard({ trial, isAnalyzing, onAnalyze, onViewDetails }: TrialCardProps) {
  ...
  const recColor = hasResult ? getRecommendationColor(matchResult.recommendation) : "hsl(var(--muted-foreground))";
  ...
  return (
    <div
      className="bg-card rounded-xl border border-border p-4 transition-all duration-200 hover:border-[rgba(13,148,136,0.3)]"
      style={{ borderColor: hasResult ? `${recColor}30` : undefined }}
    >
      <div className="flex items-start gap-3 mb-3">
        {hasResult ? (
          <ScoreRing score={matchResult.overall_score} color={recColor} />
        ) : (
          <div className="w-[52px] h-[52px] rounded-full border-2 border-border flex items-center justify-center flex-shrink-0">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-1">
            ...
            {hasResult && (
              <Badge
                variant={
                  matchResult.recommendation === "strong_match" ? "emerald"
                  : matchResult.recommendation === "possible_match" ? "teal"
                  : matchResult.recommendation === "unlikely" ? "amber"
                  : "rose"
                }
                size="sm"
              >
          ...
          <h3 className="text-sm font-semibold text-foreground leading-snug line-clamp-2">{trial.title}</h3>
        </div>
      </div>

      <div className="flex items-center justify-between mb-3">
        <span className="text-xs text-muted-foreground truncate max-w-[60%]">{trial.sponsor}</span>
        <a
          href={trial.url}
          target="_blank"
        ...
        <div className="mb-3 space-y-1">
          {matchResult.key_concerns.slice(0, 2).map((concern, idx) => (
            <div key={idx} className="flex items-start gap-1.5">
              ...
              <span className="text-xs text-muted-foreground leading-relaxed">{truncate(concern, 80)}</span>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="flex gap-2 mt-3">
        ...
        >
          {isAnalyzing ? (
            <><LoadingSpinner size="sm" color="#0D9488" />Analyzing...</>
          ) : hasResult ? "Re-analyze" : "Analyze Eligibility"}
        </button>

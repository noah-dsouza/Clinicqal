...
export function IntakeShell() {
  ...
  const form = useIntakeForm(() => router.push("/dashboard"));
  ...
  return (
    ...
    >
      ...
      <div className="w-full max-w-3xl px-4 pt-12 pb-8">
        ...
        <h1 className="text-3xl font-bold text-foreground mt-4">Build Your Health Profile</h1>
        ...
        <div className="mt-4 flex items-center gap-3">
          ...
          <span className="text-xs text-muted-foreground">Auto-fills your profile with AI</span>
        </div>
            ...
            {stepNumbers.map((step) => {
              ...
              return (
                <React.Fragment key={step}>
                  <button
                    ...
                    className={`flex items-center justify-center rounded-full text-xs font-bold transition-all duration-300 ${
                      isActive
                        ...
                        : "w-7 h-7 bg-muted text-muted-foreground border border-border"
                    }`}
                  >
                    {isComplete ? (
                  ...
                  {step < 6 && (
                    <div className={`flex-1 h-0.5 rounded transition-all duration-500 ${step < form.currentStep ? "bg-[#22C55E]" : "bg-border"}`} />
                  )}
                </React.Fragment>
              );
            })}
            ...
            {stepNumbers.map((step) => (
              <div
                ...
                className={`text-xs transition-colors duration-200 ${
                  step === form.currentStep ? "text-[#0D9488] font-medium" : step < form.currentStep ? "text-[#22C55E]" : "text-muted-foreground"
                }`}
                style={{ width: `${100 / 6}%` }}
              >
      ...
      <div className="w-full max-w-3xl px-4 pb-12">
        <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
          ...
          {form.error && (
            <div className="mx-6 mb-4 px-4 py-3 rounded-lg bg-[rgba(239,68,68,0.06)] border border-[rgba(239,68,68,0.2)] text-destructive text-sm">
              {form.error}
            </div>
          )}

          <div className="px-6 md:px-8 pb-6 md:pb-8 flex items-center justify-between border-t border-border pt-5">
            <button
              onClick={form.prevStep}
              disabled={form.currentStep === 1}
              className="px-5 py-2.5 rounded-lg border border-border text-muted-foreground text-sm font-medium transition-all hover:bg-muted hover:text-foreground disabled:opacity-40 disabled:cursor-not-allowed"
            >
              ← Back
            </button>
            <span className="text-xs text-muted-foreground">Step {form.currentStep} of {form.totalSteps}</span>

...
export function Step1Demographics({ data, onChange }: Props) {
  return (
    <div className="fade-in-up">
      <h2 className="text-xl font-bold text-foreground mb-1">Demographics</h2>
      <p className="text-sm text-muted-foreground mb-6">Basic demographic information helps match you with eligible trials.</p>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label className={labelCls}>Age *</label>
          <input type="number" className={inputCls} placeholder="e.g. 45" min={0} max={130}
            value={data.age || ""} onChange={(e) => onChange({ age: parseInt(e.target.value) || 0 })} />
        </div>

        <div>
          <label className={labelCls}>Biological Sex *</label>
          <select className={inputCls} value={data.sex} onChange={(e) => onChange({ sex: e.target.value as "male" | "female" | "other" })}>
        ...
        <div>
          ...
          <select className={inputCls} value={data.ethnicity} onChange={(e) => onChange({ ethnicity: e.target.value })}>
        ...
        <div>
          ...
          <input type="text" className={inputCls} placeholder="e.g. 90210" maxLength={10}
            value={data.zip_code} onChange={(e) => onChange({ zip_code: e.target.value })} />
        </div>

        <div>
          <label className={labelCls}>Weight (kg) *</label>
          <input type="number" className={inputCls} placeholder="e.g. 75" min={1} max={500} step={0.1}
            value={data.weight_kg || ""} onChange={(e) => onChange({ weight_kg: parseFloat(e.target.value) || 0 })} />
        </div>

        <div>
          <label className={labelCls}>Height (cm) *</label>
          <input type="number" className={inputCls} placeholder="e.g. 175" min={50} max={280}
            value={data.height_cm || ""} onChange={(e) => onChange({ height_cm: parseFloat(e.target.value) || 0 })} />
        </div>
      </div>
